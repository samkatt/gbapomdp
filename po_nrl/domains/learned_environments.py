...
import numpy as np
from po_nrl.agents.neural_networks.neural_pomdps import (
    DynamicsModel,
    get_optimizer_builder,
)
from po_nrl.domains.gridverse_domain import GridverseDomain
from po_nrl.environments import ActionSpace, SimulationResult, Simulator, TerminalState
...
def train_from_samples(
    ...
    Args:
         theta: (`po_nrl.agents.neural_networks.neural_pomdps.DynamicsModel`):
...
class BADDr(Simulator, POBNRLogger):
    ...
    def simulation_step(self, state: AugmentedState, action: int) -> SimulationResult:
        ...
        new_model = copy.deepcopy(state.model)
        self.update_theta(new_model, state.domain_state, action, new_domain_state, obs)

        return SimulationResult(self.AugmentedState(new_domain_state, new_model), obs)
