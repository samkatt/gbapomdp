...
from general_bayes_adaptive_pomdps.agents import Agent
from general_bayes_adaptive_pomdps.environments import Environment


def run_episode(env: Environment, agent: Agent, conf) -> float:
    """runs a single episode of the agent in the environmnet

    NOTE: assumes env and agent has been reset

    Args:
         env: (`general_bayes_adaptive_pomdps.environments.Environment`):
         agent: (`general_bayes_adaptive_pomdps.agents.agent.Agent`):
         conf: configurations
