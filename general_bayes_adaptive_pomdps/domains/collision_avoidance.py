...
class CollisionAvoidance(Domain):
    ...
    def __init__(
        self,
        domain_size: int,
        obstacle_policy: Optional[Tuple[float, float, float]] = None,
    ):
        ...
        self._block_policy = obstacle_policy if obstacle_policy else (0.25, 0.5, 0.25)

        assert (
            abs(np.sum(self._block_policy) - 1) < 0.0001
