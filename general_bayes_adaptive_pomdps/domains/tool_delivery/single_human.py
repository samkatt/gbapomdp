...
class ObjSearchDelivery(gym.Env):
    ...
    def __init__(self, 
        ...
        self.BWPs = []
        self.BWPs.append(BeliefWayPoint('WaitWorkSpot', 0, 6.0, 2.0))
        self.BWPs.append(BeliefWayPoint('ReceiveToolSpot', 1, 1.5, 3.5))
        self.BWPs.append(BeliefWayPoint('ToolDeliverySpot', 2, 6.0, 3.0))
    ...
    def createHumans(self):
        ...
        #-----------------initialize a human
        Human = AgentHuman(0, self.n_steps_human_task, self.human_speed[0], list(range(self.n_objs)))
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            self.fetchtrans = rendering.Transform(translation=(self.agents[1].xcoord*100, self.agents[1].ycoord*100))
            ...
            self.fetchtrans_c = rendering.Transform(translation=(self.agents[1].xcoord*100, self.agents[1].ycoord*100))
            ...
            self.arm2trans = rendering.Transform(translation=(self.agents[1].xcoord*10000+49, self.agents[1].ycoord*100), rotation = -90/180*np.pi)
            ...
            self.arm2trans_c = rendering.Transform(translation=(self.agents[1].xcoord*10000+48, self.agents[1].ycoord*100), rotation = -90/180*np.pi)
        ...
        self.fetchtrans.set_translation(self.agents[1].xcoord*100, self.agents[1].ycoord*100)
        ...
        self.arm2trans_c.set_translation(self.agents[1].xcoord*10000+48, self.agents[1].ycoord*100)
        self.arm2trans.set_translation(self.agents[1].xcoord*10000+49, self.agents[1].ycoord*100)


        self.pass_objs = 0

        if self.agents[1].cur_action is not None and \
                self.agents[1].cur_action.idx == 1 and \
                self.agents[1].cur_action_time_left <= 0.0 and \
                not self.agents[1].serving_failed and self.pass_objs < self.n_objs:
                    self.pass_objs += 1
                    self.arm0.set_color(0.0, 0.0, 0.0)
                    self.arm0_c.set_color(0.5,0.5,0.5)

        elif self.agents[1].cur_action is not None and \
                self.agents[1].cur_action.idx == 2 and \
                self.agents[1].cur_action_time_left <= 0.0 and \
                not self.agents[1].serving_failed and self.pass_objs < self.n_objs:
                    self.pass_objs += 1
                ...
                self.agents[1].cur_action.idx > 2 and \
                np.sum(self.agents[1].count_found_obj) < self.n_objs:
                    self.arm2trans_c.set_translation(self.agents[1].xcoord*100+48, self.agents[1].ycoord*100)
                    self.arm2trans.set_translation(self.agents[1].xcoord*100+49, self.agents[1].ycoord*100)
...
class ObjSearchDelivery_v4(ObjSearchDelivery):
    ...
    def create_turtlebot_actions(self):
        ...
        self.action_space_T = spaces.Discrete(self.n_objs + 2)
        ...
        self.T_MAs.append(MacroAction('Go_WR', 0, expected_t_cost=None, ma_bwpterm=0))

        for i in range(self.n_objs):
            self.T_MAs.append(MacroAction(f'Get_Tool_{i}', i + 1, expected_t_cost=None, ma_bwpterm=1))
        
        self.T_MAs.append(MacroAction('Deliver', self.n_objs + 1, expected_t_cost=None, ma_bwpterm=2))
    ...
    def create_fetch_actions(self):
        ...
        self.F_MAs = []
        for i in range(self.n_objs):
            F_MA = MacroAction(f'Get_Tool_{i}', i, expected_t_cost=self.fetch_look_for_obj_tc)
            self.F_MAs.append(F_MA)

    def createAgents(self):

        #-----------------initialize Two Turtlebot agents
        Turtlebot1 = AgentTurtlebot_v4(0, 3.0, 1.5, self.BWPs_T0, self.T_MAs, self.n_objs, speed=self.TB_move_speed,
        ...
        self.agents = [Turtlebot1, Fetch_robot]
    ...
    def step(self, actions):
        ...
        if not self.agents[1].cur_action_done:
            reward = self.agents[1].step(self.agents[1].cur_action.idx, self.agents)
            cur_actions.append(self.agents[1].cur_action.idx)
        else:
            reward = self.agents[1].step(actions[1], self.agents)
            cur_actions.append(actions[1])
        rewards += reward
        ...
        if self.rendering:
            ...
            print("Fetchrobot \t action \t\t{}".format(self.agents[1].cur_action.name))
            print("           \t action_t_left \t\t{}".format(self.agents[1].cur_action_time_left))
            print("           \t action_done \t\t{}".format(self.agents[1].cur_action_done))
            print("           \t is_serving \t\t{}".format(self.agents[1].serving))
            print("           \t serving_failed \t{}".format(self.agents[1].serving_failed))
    ...
    def _getobs(self):
        ...
        for idx, agent in enumerate(self.agents[0:2]):
            ...
            if BWP is None or BWP.idx >= len(self.BWPs_T0)-2:
                T_obs_4 = self.agents[1].ready_objs
            else:
                T_obs_4 = np.zeros(2)
        ...
        if not self.agents[1].cur_action_done:
            ...
            if self.rendering:
                ...
                print(" ")
                print("          " + " \t Found_objs  \t{}".format(self.agents[1].found_objs))
        else:
            # get observation about which objects are ready
            F_obs_0 = np.zeros(self.n_objs)
            for obj_idx in self.agents[1].found_objs:
                F_obs_0[obj_idx] += 1
    ...
    def get_state(self):
        ...
        if self.agents[1].cur_action is not None:
            if self.agents[1].cur_action.idx in [1,2] and \
                    not self.agents[1].cur_action_done and \
                    len(self.agents[1].found_objs) > 0:
                        obj_inhand[self.agents[1].found_objs[0]] = 1.0
        assert obj_inhand.sum() <= 1, "More than one object in fetch's hand ..."
        state += obj_inhand.tolist()

        # fetch's action status
        if self.agents[1].cur_action is not None:
            state.append(self.agents[1].cur_action_time_left / self.agents[1].cur_action.t_cost)
        else:
            state.append(0.0)
