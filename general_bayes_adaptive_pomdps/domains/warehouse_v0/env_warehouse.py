...
class EnvWareHouse(object):
    ...
    def reset(self, state=None):
        ...
        # current-tool: 0, 1, 2, 3, 4 (5) 0 means carrying nothing
        # human-status: working or waiting (2)
        # task-stage: 0 (init), 1, 2, 3, 4 (5)
        ...
        if state is not None:
            ...
            (human_status, desired_tool, human_step_cnt) = state[2:5]
            assert 0 <= desired_tool <= 4
            try:
                # if desired_tool is 0 --> the real desired tool is #1
                task_stage = self.correct_tool_order.index(desired_tool + 1)
            except ValueError:
                task_stage = 0
            assert Human.WAITING_TOOL <= human_status <= Human.WORKING
            assert 0 <= task_stage <= 4
            assert 0 <= human_step_cnt <= 3
        else:
            (bot_loc_id, bot_tool) = (0, 0) # (at workroom, free)
            (human_status, task_stage, human_step_cnt) = (0, 0, 0) # (waiting, init stage, init cnt)
    ...
    def get_state(self):
        ...
        # the desired tool will not change when all tools are delivered
        if self.task_stage == 4:
            desired_tool = self.correct_tool_order[-1]
        else:
            desired_tool = self.correct_tool_order[self.task_stage] - 1

        human_state = [self.human.current_status, desired_tool, self.human.time_step]

        return np.array(bot_state + human_state, dtype=int)
    ...
    def is_terminal_state(self, state):
        ...
        return desired_tool == self.correct_tool_order[-1] and human_status == Human.WORKING
...
if __name__ == "__main__":
    ...
    env = EnvWareHouse(correct_tool_order=[2, 1, 3, 4])
    env.reset()
    env.render()
    time.sleep(2)

    action_list = [Go_to_TR, Get_tool_2, Go_to_WR, Deliver_tool, Go_to_TR, Get_tool_1, Go_to_WR, Deliver_tool]
