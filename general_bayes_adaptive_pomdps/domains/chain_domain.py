...
class ChainDomain(Environment, Simulator, POBNRLogger):
    ...
    def __init__(self, size: int, encoding: EncodeType):
        ...
        self._action_mapping = np.random.binomial(1, 0.5, self.size)
    ...
    def state_space(self) -> DiscreteSpace:
        """ `general_bayes_adaptive_pomdps.misc.DiscreteSpace` ([size,size])"""
    ...
    def observation_space(self) -> DiscreteSpace:
        """ a `general_bayes_adaptive_pomdps.misc.DiscreteSpace`, depends on `one_hot_observation` flag for `this`"""
    ...
    def simulation_step(self, state: np.ndarray, action: int) -> SimulationResult:
        ...
        assert self.action_space.contains(
            action
        ), f"expecting action A or B, not {action}"
