    ...
    def step(self, action, humans):
        ...
        if self.cur_action.expected_t_cost != 1:
            ...
            if dist <= self.speed:
                ...
                # Deliver_Tool actions
                if action in [self.n_objs, self.n_objs + 1]:
        ...
        if self.cur_BWP is not None and \
            ...
            (action in [self.n_objs, self.n_objs + 1]):  # Deliver_Tool actions

            assert len(humans) == 2
            target_human = action - self.n_objs
            human = humans[target_human]
        ...
        if action < self.n_objs and self.reached_receipt_spot:
            ...
            if self.fetch.cur_action_done:
                if self.fetch.tool_found:
                    obj_idx = action
                    assert self.objs_in_basket[obj_idx] <= 1
