...
class StateEncoding(Protocol):
    ...
    @staticmethod
    def construct(descr: str, rep: DefaultStateRepresentation) -> "StateEncoding":
...
class CompactStateEncoding(StateEncoding):
    ...
    def encode(self, s: GverseState) -> np.ndarray:
        ...
        return np.concatenate([state["grid"][:, :, 3].flatten(), state["agent"][:3]])
...
class OneHotStateEncoding(StateEncoding):
    ...
    def decode(self, s: np.ndarray) -> Tuple[np.ndarray, GversePosition, Orientation]:
        ...
        Assumes one - hot encoded state as implemented in `encode` and extracts
        the grid and position & orientation of the agent
...
class ObservationModel(DynamicsModel.ObsModel):
    """known observation model for Gridverse to use by `DynamicsModel`"""

    def __init__(self, obs_size: int, encoding: StateEncoding, max_item_index: int):
...
class GridverseDomain(Domain):
    ...
    def reward(self, state: np.ndarray, action: int, new_state: np.ndarray) -> float:
        ...
        Reward space of this domain is either 0 or 1:
    ...
    def sample_transition(
        ...
        - the state from the 'reset' function(which we assume generates all reachable states!)
    ...
    def create_dynamics_observation_model(self) -> DynamicsModel.ObsModel:
