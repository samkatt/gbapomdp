...
class WareHouseV1(Domain):
    ...
    def __init__(
        ...
        # Human Status: [0, 1, 2, 3, UNKNOWN=4] x Tool-i Status: [0, 1, 2] x Agent's location [0, 1] 
        ...
        num_obss = self._max_human_status * self._max_tool_status**NUM_TOOLS * self._max_location
        ...
        self._state_lst = list(state_space.elems)
        ...
        self._obs_lst = list(obs_space.elems)
    ...
    def _prepare_obs(self, state):
        ...
        obs.tool_status_0.value = state.tool_status_0.value
        obs.tool_status_1.value = state.tool_status_1.value
        obs.tool_status_2.value = state.tool_status_2.value
...
def create_tabular_prior_counts(
    ...
    # For generating the true transition and observation tables
    state_space = one_to_one.JointNamedSpace(
        human_status=one_to_one.RangeSpace(_max_human_status - 1),
    ...
    for s in state_space.elems:
        for a in action_space.elems:
            if isStateValid(s):
                ...
                s_p_1 = simulation_result.state
                o = simulation_result.observation

                next_s = env._state_lst[s_p_1[0]]
                next_o = env._obs_lst[o[0]]

                if isStateValid(next_s) and isObsValid(next_o):
                    T[s.idx, a.idx, s_p_1[0]] = 1.0
                    O[a.idx, s_p_1[0], o[0]] = 1.0
