...
from general_bayes_adaptive_pomdps.domains.ordered_tool_delivery_four_tools.macro_factored_all import ObjSearchDelivery_v4 as EnvToolDelivery
...
class ToolDeliveryV0Prior(DomainPrior):
    ...
    def sample(self, prior=None) -> Domain:
        ...
        if prior:
            ...
            perm = list(permutations([0, 2, 1, 3]))
            correct_tool_order = random.choice(perm)
        return ToolDeliveryV0(list(correct_tool_order))

if __name__ == "__main__":
    env = ToolDeliveryV0(correct_tool_order=[3, 2, 1, 0], render=False)
    env.reset()

    index_2_str = ['Get_Tool_0', 'Get_Tool_1', 'Get_Tool_2', 'Get_Tool_3', 'Deliver']
    ...
    if optimal:
        ...
        act_list = [GET_TOOL_3] + [DELIVER] + [GET_TOOL_2] \
                 + [DELIVER] + [GET_TOOL_1] + [DELIVER] + \
                   [GET_TOOL_0] + [DELIVER]
