...
class DynamicsModel:
    ...
    class ONet(NN):
        ...
        def model(
            self, action: int, next_state: np.ndarray
        ) -> List[float]:
            ...
            action_next_state = (
                torch.from_numpy(
                    np.concatenate(
                        [self.action_space.one_hot(action), next_state]
                    )
                )
                .to(device())
                .float()
            ...
            with torch.no_grad():
                logits = self.net(action_next_state)
        ...
        def sample(
            ...
            """
            obs_model = self.model(action, next_state)
    ...
    def batch_update(
        ...
        if conf != DynamicsModel.FreezeModelSetting.FREEZE_O:
            loss_o += self.o.batch_train(actions, next_states, obs)
    ...
    def sample_observation(
        ...
        """
        return self.o.sample(action, next_state, num)
    ...
    def observation_model(
        ...
        return self.o.model(action, next_state)
