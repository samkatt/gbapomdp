...
def train_from_samples(
    ...
    for _ in range(num_epochs):
        ...
        new_loss_t, new_loss_o = theta.batch_update(
            np.array(states, dtype=np.float),
            np.array(actions),
            np.array(new_states, dtype=np.float),
            np.array(observations, dtype=np.int)
        )
...
def sample_transitions_uniform(
    ...
    while True:
        ...
        next_state, observation = prior.domain_simulation_step(state, action)
