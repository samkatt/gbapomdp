...
def extract_statistics(input_content: List[np.ndarray]) -> List[Dict[str, np.ndarray]]:
    ...
    for content in input_content:
        ...
        if len(content.shape) == 1:  # planning results
            ...
            assert (
                not contains_learning_result
            ), "found both plannign and learning results"
...
def combine_var_and_mean(
    ...
    stat2 = (
        statistics[1]
        if (len(statistics) == 2)
        else combine_var_and_mean(statistics[1:])
    )
    ...
    var = real2sample(
        (
            ...
            + stat2["n"]
            * (sample2real(stat2["var"], stat2["n"]) + stat2["mu"] ** 2)  # noqa W503
        )
        / n
        - mu ** 2,
        n,
