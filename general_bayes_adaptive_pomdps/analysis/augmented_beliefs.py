...
from general_bayes_adaptive_pomdps.agents.planning.belief import BeliefAnalysis
...
def tiger_model_analysis(belief: Belief) -> List[Tuple[str, Union[float, np.ndarray]]]:
    ...
    hear_correct_1 = np.array(
        [
            belief().model.observation_model(right_state, open_door, right_state)[0][1]
            for _ in range(1000)
        ]
    )
...
def ca_transition_analysis(
    belief: Belief, size: int
) -> List[Tuple[str, Union[float, np.ndarray]]]:
    ...
    correct_agent_x_prob = np.array(
        [
            belief().model.transition_model(non_terminal_states[i], actions[i])[0][
                non_terminal_states[i][0] - 1
            ]
    ...
    correct_agent_y_prob = np.array(
        [
            belief().model.transition_model(non_terminal_states[i], actions[i])[1][
                correct_next_y[i]
            ]
            for i in range(sample_size)
        ]
    ...
    impossible_obst_transition_prob = np.concatenate(
        [
            belief().model.transition_model(
                non_terminal_obst_bottom_states[i], actions[i]
            )[2][2:]
            for i in range(sample_size)
        ]
    )
    ...
    obst_stay_mid_prob = np.array(
        [
            belief().model.transition_model(
                non_terminal_obst_middle_states[i], actions[i]
            )[2][mid]
            for i in range(sample_size)
        ]
    )
    ...
    correct_x_pos_prob = np.array(
        [
            belief().model.observation_model(
                non_terminal_states[i], actions[i], next_states[i]
            )[0][next_states[i][0]]
            for i in range(sample_size)
        ]
    )
...
def rr_observation_analysis(
    ...
    next_states = np.array(
        [models[i].sample_state(states[i], actions[i]) for i in range(size)]
    )

    correct_observation_prob = np.array(
        [
            models[i].observation_model(states[i], actions[i], next_states[i])[0][
                next_states[i][RoadRacer.get_current_lane(next_states[i])]
            ]
            for i in range(size)
        ]
    )
...
def rr_agent_lane_change(
    ...
    actions = np.random.randint(low=0, high=3, size=size)
    correct_next_lane = np.clip(
        a=states[:, -1] + actions - 1, a_min=0, a_max=num_lanes - 1
    )

    correct_lane_prob = np.array(
        [
            belief().model.transition_model(states[i], actions[i])[-1][
                correct_next_lane[i]
            ]
            for i in range(size)
        ]
...
def rr_lane_advances(
    ...
    for i in range(size):
        ...
        potential_next_lane[
            i, np.arange(num_lanes), (states[i, :-1] - 1) % length
        ] = True
    ...
    car_block_prob = np.array(
        [
            belief().model.transition_model(states[i], actions[i])[
                RoadRacer.get_current_lane(states[i])
            ][
                1
            ]  # stay
            for i in range(size)
    ...
    car_reappear_prob = np.array(
        [
            belief().model.transition_model(states[i], actions[i])[random_lanes[i]]
            for i in range(size)
        ]
    )
...
def count_unique_models(belief: Belief) -> List[Tuple[str, Union[float, np.ndarray]]]:
